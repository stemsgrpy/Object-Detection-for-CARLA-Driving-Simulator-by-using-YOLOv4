...
def fit_ont_epoch(net, yolo_losses, epoch, epoch_size, epoch_size_val, gen, genval, Epoch, cuda):
    ...
    for iteration, batch in enumerate(gen):
        ...
        if iteration >= epoch_size:
            break
        # input,    numpy.ndarray(bs, 3, input_shape[0], input_shape[1])
        # targets,  numpy.ndarray(bs)
        ...
        with torch.no_grad():
            if cuda:
                ...
                targets = [Variable(torch.from_numpy(ann).type(torch.FloatTensor)) for ann in targets]
        optimizer.zero_grad()
        outputs = net(images) # head1, head2, head3

        losses = []
        for i in range(3): # feature_length[i]
            # outputs[i] -> torch.Size([bs, 3*(5+num_classes), feature_length[i], feature_length[i]])
...
if __name__ == "__main__":
    ...
    annotation_path = '2007_train.txt'            # your dataset [change content]

    anchors_path = 'model_data/yolo_anchors.txt'
    classes_path = 'model_data/voc_classes.txt'   # your clesses [change content]
    ...
    model_path = "model_data/yolo4_voc_weights.pth" # pre-traing model
    ...
    # dataset setting -> 10% validation, 90% training
    val_split = 0.1
    ...
    '''Unfreeze whole network for training'''
    lr = Unfreeze_lr
    optimizer = optim.Adam(net.parameters(),lr,weight_decay=5e-4)
    if Cosine_lr:
        lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=5, eta_min=1e-5)
