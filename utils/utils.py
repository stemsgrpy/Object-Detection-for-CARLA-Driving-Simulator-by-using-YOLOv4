...
class DecodeBox(nn.Module):
    ...
    def forward(self, input):
        ...
        stride_h = self.img_size[1] / input_height  # feature_length[i]
        stride_w = self.img_size[0] / input_width   # feature_length[i]
        ...
        # map output to input_shape (608x608)
...
def non_max_suppression(prediction, num_classes, conf_thres=0.5, nms_thres=0.4):
    ...
    for image_i, image_pred in enumerate(prediction): #
        ...
        # foreground phase1_1 (confidence threshold)
        ...
        # foreground phase1_2 (class & class confidence)
        ...
        for c in unique_labels:
            # foreground phase2_1 (obtain speific class (c))
            detections_class = detections[detections[:, -1] == c]
            # foreground phase2_2 (sort speific class base on thier confidence)
            _, conf_sort_index = torch.sort(detections_class[:, 4], descending=True)
            detections_class = detections_class[conf_sort_index]

            # nms
            max_detections = []
            while detections_class.size(0): # object contain not empty
                # foreground phase2_3 (start nms by max confidence of speific class)
                max_detections.append(detections_class[0].unsqueeze(0))
                if len(detections_class) == 1:
                    break
                # foreground phase2_4 (ious between max and others)
                ious = bbox_iou(max_detections[-1], detections_class[1:])
                # foreground phase2_5 (delete silmar box base on nms threshold)
